...
const ShopItemGrid: React.FC<ShopItemGridProps> = ({ shopItems, isLoading, isBuying, onBuy }) => {
  return (
          ...
          {shopItems?.map((item: any) => (
            ...
            >
              ...
              <motion.button
                ...
                onClick={() => onBuy(item)}
                disabled={isBuying}
                className="relative z-10 w-full py-3 rounded-lg font-bold uppercase tracking-widest text-sm flex items-center justify-center gap-2 transition-all bg-purple-500/10 border border-purple-500/30 text-purple-400 hover:bg-purple-500 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Buy for {item.cost}
