...
function LevelCompleted_PopUp({ subj, lessonId, LevelId, heartsRemaining, setLevelComplete, resetHearts }: {subj: string; lessonId: string; LevelId: string; heartsRemaining: number; setLevelComplete: (val: boolean) => void; resetHearts: () => void}): React.ReactNode {
...
const LevelData = useMemo(() => {
  const lesson = levelsData.find((l: { id: string; Levels?: { id: string }[] }) => l.id === lessonId);
  return lesson?.Levels?.find((lv: { id: string }) => lv.id === LevelId) || null;
