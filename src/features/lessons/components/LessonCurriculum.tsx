  ...
  return (
    <div className="w-full lg:w-[65%] flex flex-col gap-6">
      <div className="flex items-center justify-between px-1">
        <h2 className="text-white font-exo text-2xl font-bold tracking-tight">
          Curriculum
        </h2>
      </div>
            ...
            <div key={i} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                {[1, 2].map((j) => (
                  <div
                    key={j}
                    className="h-[5.5rem] w-full bg-[#0d0d12] border border-[#1e1e2e] rounded-xl animate-pulse flex items-center p-4 gap-4"
                  >
        ...
        <div className="flex flex-col gap-8">
          {levelsData.map((lesson: any) => (
            <div key={lesson.id} className="flex flex-col gap-4">
              <h3
                className={`font-exo text-xl font-bold text-white border-b border-[#2a2a3c] pb-2 ${config.theme.textLight}`}
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {/* Icon / Lock Box */}
                        <div
                          className={`w-14 h-14 shrink-0 rounded-lg flex items-center justify-center border transition-colors ${
                            isUnlocked
                        ...
                        <div className="flex-1 min-w-0 text-center sm:text-left">
                          <h4
                            className={`font-exo text-lg font-bold tracking-tight truncate ${isUnlocked ? "text-white" : "text-slate-400"}`}
                          >
                            {level.title}
                          </h4>
                        ...
                        {isExpanded && isUnlocked && (
                          ...
                          >
                            <div className="pl-6 sm:pl-[4.5rem] pr-2 py-2 flex flex-col gap-2 relative">
                                  ...
                                  return (
                                    <div
                                      ...
                                      onClick={() => {
                                        if (isStageUnlocked) {
                                          navigate(
                                            `/Main/Lessons/${subject}/${lesson.id}/${level.id}/${stage.id}/${stage.type}`,
                                          );
                                        }
                                      }}
                                    >
                                      <div
                                        className={`w-3 h-3 rounded-full absolute -left-[1.35rem] sm:-left-[2.6rem] border-4 border-[#06060a] ${
                                          isStageUnlocked
                                            ? `${config.theme.bg} ${config.theme.shadow}`
                                            : "bg-[#2a2a3c]"
                                        }`}
                                      />
                                      ...
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                          {isBoss && (
                                            <span className="text-red-400 text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-red-500/10 border border-red-500/20">
                                              Challenge
                                            </span>
                                          )}
                                          <p
                                            className={`font-exo font-bold ${isStageUnlocked ? "text-slate-200" : "text-slate-500"}`}
                                          >
                                            {stage.title}
                                          </p>
                                        </div>
                                      ...
                                      <div className="shrink-0 text-slate-500">
                                        {!isStageUnlocked ? (
                                          ...
                                          <IoPlayOutline
                                            size={20}
                                            className={`${config.theme.textLight} transition-colors`}
                                          />
                                        )}
                                      </div>
                                    </div>
                              ...
                              {level.stages?.filter(
                                (stage: any) => stage.type === "Lesson",
                              ).length === 0 && (
                                <p className="text-slate-500 italic text-sm ml-4">
