...
const LessonHeader: React.FC<LessonHeaderProps> = ({ subject }) => {
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 bg-gradient-to-r ${config.theme.bgGradient} via-transparent to-transparent pointer-events-none`}
      />

      <div className="relative z-10 w-full lg:w-3/4 flex flex-col gap-4">
        <div
          className={`inline-flex items-center gap-2 px-3 py-1 rounded-md text-xs font-semibold uppercase tracking-widest w-max ${config.theme.badge}`}
        >
          {config.header.badge}
        </div>
      ...
      <div
        className={`relative z-10 w-32 h-32 lg:w-48 lg:h-48 shrink-0 ${config.theme.dropShadow}`}
      >
