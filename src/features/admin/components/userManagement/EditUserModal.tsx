...
import { useDeleteSpecificAchievement } from "../../hooks/useDeleteSpecificAchievement";
import DeleteConfirmationModal from "./DeleteConfirmModal";
import useFetchLevelsData from "@/services/api/useFetchLevelsData";
import { User, SubjectLesson } from "../../types";

const categories: string[] = ["Html", "Css", "JavaScript", "Database"];
...
const EditUserModal = ({ visibility, closeModal, uid }: EditUserModalProps): React.ReactElement | null => {
  const queryClient = useQueryClient();
  const deleteProgress = useDeleteProgress();
  const deleteSpecificAchievement = useDeleteSpecificAchievement();
  const [deleteTarget, setDeleteTarget] = useState<DeleteTarget | null>(null);

  const users = queryClient.getQueryData<User[]>(["allUser"]) || [];
  ...
  const levelsQueries = categories.reduce<Record<string, any>>((acc, category) => {
                ...
                <AnimatePresence mode="wait">
                  ...
                  {activeTab === "info" && (
                    ...
                    >
                      ...
                      {(["username", "bio", "coins", "exp", "userLevel"] as const).map((field) => (
                        <div key={field}>
                    ...
                    >
                      ...
                      <div className="space-y-4">
                        {categories.map((category) => {
                          const { levelsData = [], isLoading } = levelsQueries[category] as {
                          ...
                          const percent = Math.min((completed / (totalLevels || 1)) * 100, 100);
                  ...
                  {activeTab === "achievements" && (
                    ...
                    >
                      ...
                      {Object.keys(achievements).length > 0 ? (
                        <div className="space-y-4">
                          {Object.entries(achievements).map(([category, ach]) => {
                            const maxAchievement = 10;
                            const percent = Math.min((ach.quantity / maxAchievement) * 100, 100);
