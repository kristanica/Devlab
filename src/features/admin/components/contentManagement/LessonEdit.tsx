...
function LessonEdit({
  ...
  stageId,
  setShowForm,
}: LessonEditProps): React.ReactElement {
  ...
  const fetchStage = async (): Promise<void> => {
    try {
      ...
      if (stageSnap.exists()) {
        setStageData(stageSnap.data() as Stage);
      } else {
        setStageData(null);
      }
    } catch (error) {
  ...
  useEffect(() => {
    dispatch({
      ...
      payload: {
        ...
        codingInterface: {
          html: stageData?.codingInterface?.html ?? "",
          css: stageData?.codingInterface?.css ?? "",
          js: stageData?.codingInterface?.js ?? "",
          sql: stageData?.codingInterface?.sql ?? "",
        },
        timer: stageData?.timer ? Number(stageData.timer) : undefined,
        choices: (stageData?.choices as unknown as StageState["choices"]) || { a: "", b: "", c: "", d: "", correctAnswer: "" },
        blocks: stageData?.blocks || [],
        copyCode: stageData?.replicationFile || "", // mapping replicationFile to copyCode if needed
      },
    });
  }, [stageData, activeTab]);
    ...
    try {
      ...
      if (!hasImages) {
        ...
        response = await axios.post(
          `${import.meta.env.VITE_BACK_END}/fireBaseAdmin/editStage`,
          {
            category: subject,
            lessonId,
            levelId,
