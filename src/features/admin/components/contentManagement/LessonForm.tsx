...
function LessonForm({
  ...
  videoFile,
  setVideoFile,
}: LessonFormProps): React.ReactElement {
  ...
  const [localPreview, setLocalPreview] = useState<string>(stageData?.videoPresentation || "");
  ...
  const addBlocks = (): void => {
    ...
    const maxId = state.blocks?.length
      ? Math.max(...state.blocks.map((b: any) => Number(b.id)))
      : 0;
    ...
    <>
      ...
      <div className="bg-slate-600 px-1 my-3 py-3 rounded-2xl">
        ...
        <button
          onClick={addBlocks}
          type="button"
          className="flex flex-row justify-center bg-[#111827] border-cyan-400 border-2 p-3 rounded-2xl mt-3 w-full text-center hover:scale-105 transition duration-300"
        >
