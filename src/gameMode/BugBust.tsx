...
export interface GameModeProps {
  ...
  roundKey: number;
  gameOver: boolean;
  submitAttempt: (success: boolean) => void;
  resetHearts: () => void;
}

const BugBust: React.FC<GameModeProps> = ({ heart, roundKey, gameOver, submitAttempt, resetHearts }) => {
  const type = "Bug Bust";
  const navigate = useNavigate();
  const { consumeErrorShield } = useErrorShield();
  const { subject, lessonId, levelId, stageId } = useParams<{ subject: string; lessonId: string; levelId: string; stageId: string }>();
...
export default React.memo(BugBust);